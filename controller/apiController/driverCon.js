            ...
            if((userId) && (userId !== "") && (name) && (name !== "")){
                if(req.files){
                    ...
                    if(req.files.document_file){
                        ...
                        const uploadData = await Driver.findByIdAndUpdate(userId,{$push:{documents:{name:name, filename:uploadResult}}}, {new: true});
                            ...
                            fs.readFile(uploadedFile.tempFilePath, (err, uploadedData) => {
                                ...
                                s3.upload(params, async (err, data) => {
                                    if (err) {
                                        ...
                                        const updateData = await Driver.updateOne({userId, "documents._id": docId},{$set:{'documents.$.filename':data.Key}}, {new: true});
