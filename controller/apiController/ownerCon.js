...
module.exports={
    documentsUpload: async (req, res) => {
        try {
            const { name, userId ,documentName} = req.body;
            if((userId) && (userId !== "") && (name) && (name !== "")){
                if(req.files){
                    ...
                    if(req.files.document_file){
                        ...
                        const uploadData = await Owner.findByIdAndUpdate(userId,{$push:{documents:{name:name, filename:uploadResult}}}, {new: true});
    ...
    updateDocument: async (req, res) => {
        try {
            ...
            const owners = await Owner.findOne({_id: userId}, {"documents": 1});
            // console.log(users.documents.length);
            for(let i=0; i<owners.documents.length; i++){
                if(owners.documents[i]._id == docId){
                    if((req.files) && (req.files.document_file)){
                        const uploadedFile = req.files.document_file;
                        const allowType = ['image/png', 'image/jpeg', 'image/jpg', 'application/pdf'];
                        const uploadResult = await fileUpload(uploadedFile,owners.documents[i].name + userId.substr(userId.length - 4),allowType);
                            ...
                            fs.readFile(uploadedFile.tempFilePath, (err, uploadedData) => {
                                ...
                                s3.upload(params, async (err, data) => {
                                    if (err) {
                                        ...
                                        const updateData = await Owner.updateOne({userId, "documents._id": docId},{$set:{'documents.$.filename':data.Key}}, {new: true});
                                        // console.log(updateData);
                                        if(updateData){
                                            //// Delete Old File //////
                                            const oldFile = owners.documents[i].filename;
                                            s3.deleteObject({
                                                Bucket: process.env.BUCKET_NAME,
                                                Key: oldFile
                                            },function (err,data){});
